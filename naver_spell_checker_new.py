       ...
       ###1. 중복체크 시작###
        ...
        for json_data in json_datas[j:]:
            ...
            if (j+1)%1000 == 0:
                ...
                save_and_print(f"[{i+1}/{files_size}]-1. 중복체크 {file} {j+1}/{size}. {short} sec. {short2} sec.")
        ...
        for line in lines:
            ...
            short = times.split(".")[0] # 초 단위 까지만
            short2 = times2.split(".")[0] # 초 단위 까지만
            save_and_print(f"[{i+1}/{files_size}]-2. 맞춤법검사 {file} {j+1}/{size}, {short} sec, {short2} sec")
            line = line.replace('\n', '')
            #맞춤법 검사할 내용
            while True:
                try:
                    ...
                    save_and_print(f"Error occurred: {e}")

                    # Out of Memory 에러 또는 기타 에러 처리
                    if "Read timed out" in str(e):
                        save_and_print("Read timed out detected. Restarting the browser...")
                        dv.quit()
                        
                    else:
                        # 다른 에러가 발생한 경우 처리
                        save_and_print("An unexpected error occurred.")
                        dv.quit()
        ...
        for j in range(len(json_datas)):
            ...
            if (j+1) % 1000 == 0:
                ...
                short = times.split(".")[0] # 초 단위 까지만
                short2 = times2.split(".")[0] # 초 단위 까지만
                save_and_print(f"[{i+1}/{files_size}] 4. 필드에러 - {file}  {j+1}/{size}. {short} sec. {short2} sec")
        ...
        for json_data in json_datas[j:]:
            ...
            if (j+1) % 1000 == 0:
                ...
                short = times.split(".")[0] # 초 단위 까지만
                short2 = times2.split(".")[0] # 초 단위 까지만
                save_and_print(f"[{i+1}/{files_size}] 5. 결합 - {file}  {j+1}/{size}. {short} sec. {short2} sec")
        ###5. 결합 끝###
